...
// ---------------------------------------------------------------------------
// Clean break — incompatible legacy assessments (§19, Step 7)
// ---------------------------------------------------------------------------

/**
 * Returns true if a saved assessment was created under a schema version other
 * than the one this build understands. The v2 clean break performs no field
 * migration: such entries cannot be resumed and must be routed to the
 * 'incompatible-legacy' screen instead of RESUME_ASSESSMENT.
 */
export function isIncompatibleAssessment(entry: AssessmentIndexEntry): boolean {
...
export function importAssessmentJson(json: string): Assessment {
  ...
  if (savedVersion !== SCHEMA_VERSION) {
    throw new Error(
      ...
      `The current tool supports schema version ${SCHEMA_VERSION} only and cannot convert ` +
      'older assessments. The file cannot be imported — start a new v2 assessment instead.'
    )
  }
 ...
 *
 * For user-facing error messages when a share link is detected but fails,
 * use decodeAssessmentFromUrlWithError() instead.
 */
export async function decodeAssessmentFromUrl(encoded: string): Promise<Assessment | null> {
  const result = await decodeAssessmentFromUrlWithError(encoded)
  return result.assessment
}

/**
 * Extended version of decodeAssessmentFromUrl that returns a structured result
 * distinguishing success, schema mismatch, and corrupt/unrecognised link.
 *
 * Use this when a share link has been detected in the URL and the user needs
 * a clear explanation if it fails — e.g. "This link was generated by an older
 * version of the tool" vs. "This link appears to be corrupted."
 */
export async function decodeAssessmentFromUrlWithError(encoded: string): Promise<{
  ...
  if (!isShareLinkSupported()) {
    return {
      assessment: null,
      error:
        'Your browser does not support the compression API required to decode share links. ' +
        'Try opening the link in a current version of Chrome, Edge, or Safari.',
    }
  }
  ...
  try {
    ...
    if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
      return { assessment: null, error: 'The share link does not contain a valid assessment.' }
    }

    const obj = parsed as Record<string, unknown>
    if (!obj.schema_version || !obj.answers || !obj.property) {
      return {
        assessment: null,
        error:
          'The share link appears to be incomplete or was generated by a different tool. ' +
    ...
    if (savedVersion !== SCHEMA_VERSION) {
      return {
        assessment: null,
        error:
          `This share link was generated by an older version of the tool (schema ${savedVersion}). ` +
          `The current tool uses schema ${SCHEMA_VERSION} and cannot convert older assessments. ` +
          'Ask the sender to re-export and share the assessment as a JSON file instead, ' +
          'or start a new v2 assessment.',
      }
    }
    ...
    return {
      ...
      error:
        ...
        'Ask the sender to share the assessment as a JSON file instead.',
    }
  }
}
